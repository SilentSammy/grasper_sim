...
def compute_neccessary_forces(desired_net_force, desired_net_torque):
    PRECOMPUTED_A_PINV = [
        [0.25, 0, 0, 0, 0.125, -0.1],
        [0, 0.3187, 0, -0.02198, 0, 0],
        [0, 0.05495, 0.3333, 0.08242, 0, 0],
        [0.25, 0, 0, 0, 0.125, 0.1],
        [0, 0.3187, 0, -0.02198, 0, 0],
        [0, -0.05495, 0.3333, -0.08242, 0, 0],
        [0.5, 0, 0, 0, -0.25, 0],
        [0, 0.3626, 0, 0.04396, 0, 0],
        [0, 0, 0.3333, 0, 0, 0]
    ]
    ...
    # Construct the desired 6-vector b (local):
    b = list(desired_net_force_local) + list(desired_net_torque_local)  # b is length 6

    # Solve for f: f = A_pinv * b (f will be a 9-element vector)
    f = mat_dot_vector(PRECOMPUTED_A_PINV, b)
    
    # Update each force vector in forces with the computed components (force in local frame):
    forces[0][3:6] = f[0:3]
    forces[1][3:6] = f[3:6]
    forces[2][3:6] = f[6:9]
...
def control():
    ...
    compute_neccessary_forces(desired_net_force, desired_net_torque)
    print("D_net_F:", desired_net_force, "D_net_T:", desired_net_torque, "F:", forces, "B_F:", box_force, "B_T:", box_torque)
