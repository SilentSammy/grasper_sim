...
class Box:
    def __init__(self, box_pos=[10, 0, 0], box_rot=[0, 0, 0]):
        ...
        y_off = 2.5
        self.forces = [
            [0, 5+y_off, self.box_dims[2]/2, 0, 0, 0],
            [0, -5+y_off, self.box_dims[2]/2, 2, 0, 0],
            [0, 0-y_off, -self.box_dims[2]/2, -2, 0, 0]
        ]
        
        y_off = 1.25
        self.forces = [
            [0, 5+y_off, self.box_dims[2]/2, 0.5, 0, 0],
            [0, -5+y_off, self.box_dims[2]/2, 1.5, 0, 0],
            [0, 0-y_off, -self.box_dims[2]/2, -2, 0, 0]
        ]
        ...
        y_off = 0
        self.forces = [
            [0+x_off, 5+y_off, self.box_dims[2]/2, 0, 1, 0],
            [0+x_off, -5+y_off, self.box_dims[2]/2, 0, 1, 0],
    ...
    def draw_goal(self):
        ...
        draw_reference_frame()
        fill(0, 255, 0, 31)  # Green color with transparency
        box(self.goal_dims[0], self.goal_dims[1], self.goal_dims[2])
        popMatrix()

    def draw_force(self, force, force_scalar = 1):
    ...
    def set_necessary_forces(self, desired_net_force, desired_net_torque):
        ...
        # --- Torque allocation for rotation about Z (yaw) ---
        # Get the individual distances from the CoM in Y for the top thrusters:
        ...
        if (r0 + r1) != 0:
            ...
            F_torque0 = 0
            F_torque1 = 0
        # Apply the differential X force (assuming force[?][3] is the X component) to generate yaw torque.
        self.forces[0][3] += F_torque0  # adds force in +X at Y = +r0
        self.forces[1][3] += F_torque1  # adds (or subtracts) force in +X at Y = -r1
        # In the future, we could also apply a force in Y depending on the position of the thrusters.

        # --- Torque allocation for rotation about Y (pitch) ---
        # Assume both top thrusters have the same z coordinate.
        z_top = self.forces[0][2]
        if z_top != 0:
            # Total extra X force needed on top to generate the desired pitch torque:
            A = desired_net_torque_local[1] / 4.0  # so that 4A equals desired torque
            # Bottom thruster gets the opposite force:
            F_torque_bottom = -A
        else:
            A = 0
            F_torque_bottom = 0

        # Get Y positions of thrusters:
        y0 = self.forces[0][1]
        y1 = self.forces[1][1]
        y2 = self.forces[2][1]

        # Compute distances in Y from each top thruster to the bottom thruster.
        d0 = abs(y0 - y2)
        d1 = abs(y1 - y2)
        ...
        # Normalize weights and compute the extra force for each top thruster.
        F_torque_top_0 = A * (w0 / (w0 + w1))
        F_torque_top_1 = A * (w1 / (w0 + w1))

        # Apply the differential forces in the X direction.
